...
// GET /api/countries - public listing
router.get('/', async (req, res) => {
  try {
    ...
    if (search) {
      conditions.push(`(country_name ILIKE $${idx} OR region ILIKE $${idx})`);
    ...
    const result = await query(`
      SELECT * FROM country_profiles ${where}
      ORDER BY country_name ASC
      LIMIT $${idx} OFFSET $${idx + 1}
    `, [...params, parseInt(limit), offset]);

    // Get distinct regions for filters
...
router.get('/:id/download', async (req, res) => {
  try {
    ...
    await query('UPDATE country_profiles SET download_count = download_count + 1 WHERE id = $1', [req.params.id]);
    await query('INSERT INTO download_logs (document_type, document_id, ip_address, user_agent) VALUES ($1, $2, $3, $4)',
      ['country_profile', req.params.id, req.ip, req.get('user-agent')]);

    if (profile.file_url) {
      const filePath = path.join(process.env.UPLOAD_DIR || './uploads', profile.file_url.replace('/uploads/', ''));
      if (fs.existsSync(filePath)) {
        res.setHeader('Content-Disposition', `attachment; filename="${profile.file_name}"`);
        return res.sendFile(path.resolve(filePath));
      }
    }

    res.json({ message: 'Download tracked', file_url: profile.file_url });
  } catch (error) {
    res.status(500).json({ error: 'Server error' });
  }
});
...
router.put('/:id', authMiddleware, upload.fields([
  ...
  try {
    ...
    if (req.files?.file) {
      const file = req.files.file[0];
      file_url = `/uploads/profiles/${file.filename}`;
      file_name = file.originalname;
      file_size = file.size;
    }
    if (req.files?.thumbnail) {
      thumbnail_url = `/uploads/images/${req.files.thumbnail[0].filename}`;
    }

    const result = await query(`
      UPDATE country_profiles SET
        country_name=$1, country_code=$2, region=$3, sub_region=$4, description=$5,
        file_url=$6, file_name=$7, file_size=$8, thumbnail_url=$9, year=$10,
        population=$11, sustainability_score=$12, is_published=$13, updated_at=NOW()
